...
-----------------------------  HoareCondition  ---------------------------------
  ...
  behavior HoareCondition
      ...
      method Init ()
        waitingThreads = new List [Thread]
        -- nextToRunThreads = new List [Thread]
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
          ...
          var bitNumber, frameAddr , i,numFramesReturned: int
...
-------------------------- StartUserProcess -------------------------
  ...
  function StartUserProcess ()
    var pcb : ptr to ProcessControlBlock
        ...
        entrypoint , initUserStackTop , oldIntStat : int
    ...
    entrypoint = openFile.LoadExecutable(&pcb.addrSpace)
    fileManager.Close (openFile)

    initUserStackTop = entrypoint+pcb.addrSpace.numberOfPages*PAGE_SIZE
    ...
    BecomeUserThread(initUserStackTop,entrypoint,initSystemStackTop asInteger )
