...
_StringConst_4:
	.word	42			! length
	.ascii	"Current thread Doesn\'t hold Current Thread"
	.align
_StringConst_3:
	.word	51			! length
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if intIsZero (isHeldBy) then goto _Label_58
	load	[r14+8],r1
	load	[r1+4],r1
	cmp	r1,r0
	be	_Label_58
...
! SEND STATEMENT...
	mov	115,r13		! source line 115
	mov	"\0\0SE",r10
!   _temp_61 = &mutex_waiting_threads
	load	[r14+8],r1
	add	r1,8,r1
	store	r1,[r14+-12]
!   Prepare Argument: offset=12  value=_P_Thread_currentThread  sizeInBytes=4
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if isHeldBy == _P_Thread_currentThread then goto _Label_68		(int)
	load	[r14+8],r1
	load	[r1+4],r1
	set	_P_Thread_currentThread,r2
	load	[r2],r2
...
!   Retrieve Result: targetName=_temp_72  sizeInBytes=1
	loadb	[r15],r1
	storeb	r1,[r14+-10]
!   _temp_74 = _temp_72 XOR 0		(bool)
	loadb	[r14+-10],r1
	mov	0,r2
	xor	r1,r2,r1
	storeb	r1,[r14+-9]
!   if _temp_74 then goto _Label_71 else goto _Label_70
	loadb	[r14+-9],r1
	cmp	r1,0
	be	_Label_70
	jmp	_Label_71
...
! ASSIGNMENT STATEMENT...
	...
	mov	135,r13		! source line 135
	mov	"\0\0SE",r10
!   _temp_75 = &mutex_waiting_threads
	load	[r14+8],r1
	add	r1,8,r1
	store	r1,[r14+-24]
!   Send message Remove
...
!   _temp_76 = t + 76
	...
	add	r1,76,r1
	store	r1,[r14+-20]
!   Data Move: *_temp_76 = 2  (sizeInBytes=4)
	mov	2,r1
	load	[r14+-20],r2
	store	r1,[r2]
! SEND STATEMENT...
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if isHeldBy != _P_Thread_currentThread then goto _Label_90		(int)
	load	[r14+8],r1
	load	[r1+4],r1
	set	_P_Thread_currentThread,r2
	load	[r2],r2
